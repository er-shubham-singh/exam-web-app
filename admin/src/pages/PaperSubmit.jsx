...
const PaperSubmit = () => {
  ...
  useEffect(() => {
    // If absolutely nothing is provided, send user home.
    ...
    try {
      await dispatch(
        createPaper({
          ...
          questions: Array.isArray(state?.selected) ? state.selected : [], // use whatever was selected earlier
          isPublished: publish,
          durationMinutes: Number(duration) || 0,
        })
      );
      toast.success("Paper created successfully!");
      // brief delay so the toast is visible, then go home
